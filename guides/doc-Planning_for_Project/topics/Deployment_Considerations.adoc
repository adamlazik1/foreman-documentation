...
There are two methods for content deployment on {Project}.
The default method is *remote execution*, and the second method is the deprecated *Katello agent*.
...
* *Katello agent* {endash} Katello agent installs and updates packages.
It uses the *goferd* service, which communicates to and from the {ProjectServer}.
It is enabled and started automatically on content hosts after a successful installation of the `katello-agent` package.
+
Note that the Katello agent is deprecated and will be removed in a future {Project} release.
